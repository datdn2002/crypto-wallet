...
export default function TrendingTokensScreen() {
		...
		try {
			...
			if (res?.statusCode === 200) {
				...
				for (const chain in data) {
					const chainTokens = data[chain].tokens;
					// Assuming you want to flatten the tokens from all chains into a single array
					setTokens((prev) => [...prev, ...chainTokens]);
				}
				// setTokens(res.data);
			}
		} finally {
	...
	const data = useMemo(() => {
		let arr = [...tokens];
		if (chainFilter !== "all") {
			const chain = chainMap[chainFilter];
			arr = chain?.tokens || [];
		}
		if (segment === "gainers") {
			arr = arr.filter((t) => Number(t.market_data?.price_change_percentage_24h ?? 0) > 0);
		} else if (segment === "losers") {
			arr = arr.filter((t) => Number(t.market_data?.price_change_percentage_24h ?? 0) < 0);
		}
		arr.sort((a, b) => (b.market_data?.volume_24h ?? 0) - (a.market_data?.volume_24h ?? 0));
		return arr;
	}, [tokens, chainFilter, segment, chainMap]);
	...
	return (
		<SafeAreaView style={styles.container}>
			...
			{loading ? (
				...
				<FlatList
					data={data}
					keyExtractor={(_, index) => "tokens-" + index}
					renderItem={({ item, index }) => <TokenRow key={"tokens-" + index} item={item} />}
...
function Filters({
	...
	return (
		<View style={styles.filtersRow}>
			...
			<Chip active={segment === "all"} label="Tất cả" onPress={() => setSegment("all")} />
			<Chip active={segment === "gainers"} label="Tăng" onPress={() => setSegment("gainers")} />
			<Chip active={segment === "losers"} label="Giảm" onPress={() => setSegment("losers")} />

			<View style={{ width: 8 }} />
			<Chip active={chainFilter === "all"} label="Tất cả chain" onPress={() => setChainFilter("all")} />
			<Chip active={chainFilter === "bsc"} label="BNB Smart Chain" onPress={() => setChainFilter("bsc")} />
			<Chip active={chainFilter === "ethereum"} label="Ethereum" onPress={() => setChainFilter("ethereum")} />
			<Chip active={chainFilter === "polygon"} label="Polygon" onPress={() => setChainFilter("polygon")} />
			<Chip active={chainFilter === "base"} label="Base" onPress={() => setChainFilter("base")} />
		</View>
	);
}
